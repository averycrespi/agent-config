...
| [mailbox](agent/extensions/mailbox/README.md)                     | Automatic durable bidirectional session messaging                           | `mailbox`, optional [Script provider](agent/extensions/mailbox/README.md#script-provider), typed `mailbox.changed`           |
...
For choosing between these mechanisms and their material authorization and ownership boundaries, see [Delegation and automation](../README.md#delegation-and-automation). Monitor observations are session-bound and do not resume after shutdown, reload or navigation. Its [global/environment configuration](agent/extensions/monitor/README.md#configuration) controls cycle and lifetime ceilings; `/monitor-config` inspects the loaded policy. [Mailbox](agent/extensions/mailbox/README.md) automatically listens on each persistent session's full UUID with fixed-window batching, idle/draft/dialog gating, visibility timeouts and bounded same-ID redelivery. Ordinary delivery needs neither Monitor nor Script permission; optional Script send/list/ack remain available. `/mailbox` is read-only status and human-only `/mailbox-clear` atomically clears the current inbox without accepting obligations. Worker identity is runtime-attributed and verified through Herdr at launch; messages remain untrusted.
...
[Coordinate](agent/extensions/coordinate/README.md) provides human-enabled persistent role bindings and status/spawn/complete tools. Bare `/coordinate-enable` derives a session mailbox; one role widget identifies the coordinator or parent. Worker launch/acceptance facts persist automatically under Git's common directory; child checkpoints retain execution/evidence and TODO retains unresolved questions/actions. A bounded role reminder is refreshed before each model call. Mailbox owns automatic bidirectional delivery; Herdr owns launch/bootstrap and process inspection. Coordinate checks listener readiness without Monitor or Script setup and adds no bookkeeping scripts, automatic ACK, acceptance or replay. Installing/loading is not enabling or authority. Standalone spin-out stays standalone; historical evidence is retained without automatic adoption.

[Work-stack](agent/skills/work-stack/SKILL.md) composes spin-out, work-ticket, Herdr and Monitor for one repository and a local-only or review-ready PR boundary. Its thin entry point applies serial policy directly to Coordinate, Mailbox, TODO, Monitor and recovery mechanics, with no intermediate manager. The parent verifies predecessor commits and release evidence before advancing. Required primitives must already be loaded; installing the skill does not launch children or reload extensions. Stacked PR CI qualifies the recorded stack base, not independent readiness for main.
