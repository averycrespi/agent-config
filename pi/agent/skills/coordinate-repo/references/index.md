...
Keep current assignment membership and its coverage under the shared mailbox observer in this record. The supervision helper's registration-time member list is historical provenance, not the current admission list. Persist additions/removals and coverage references without replacing an active mailbox-scoped watcher; resolve current identities on attention. See [shared supervision](supervision.md#membership-and-catch-up).
