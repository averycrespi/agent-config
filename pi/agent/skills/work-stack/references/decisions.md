...
Reference this contract by a resolved readable path in the initial handoff and require the child to read it before work; do not paste the protocol into every handoff. Include the parent identity/checkpoint reference and actual ticket delivery authority inline. The child must:
...
After acknowledged application, replace the parent's resolved continuation intent with a durable reference to the child-owned resolution; do not retain a second decision history or copy child allowances. The child records the matching resolution and provenance in its existing checkpoint and continues under unchanged delivery criteria, serial dependencies and ownership. Decision waiting and continuation never reset parent observation, child CI or repair allowances, renew absolute deadlines, expand publication/destructive-action authority, or waive review/checks. If retained observation allowance is exhausted, request explicit additive authority before another registration; an answer alone does not replenish budgets. Advance only after the existing work-stack delivery-boundary checks pass.
