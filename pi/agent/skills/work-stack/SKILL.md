...
Retain reconciled order/authority in the [stack checkpoint](references/checkpoint.md) using its concrete three-ticket example as a replace-in-place current-state snapshot. Reference child-owned delivery records and retained history rather than copying their contents; recheck mutable facts before each launch.
...
- Parent-managed decision contract: no answer/approval from `decision_required`; investigate existing evidence, otherwise checkpoint/report request ID, ticket/run/session/current revision, question/options/recommendation/evidence and blocked work. Yield with sole ownership retained, without retrying or bypassing the mode. Parent answers within delegated authority or escalates to the user, then continues the same child; no successor on decision wait.
...
Use [parent observation accounting](references/observation.md) to prepare, attach and reconcile original host receipts and return authoritative remaining bounds. Do not write supplementary deadline/wake arithmetic in progress prose. Default parent observation allowance per child is **2 hours wall-clock from the first registration and 30 cumulative Background handoff attempts** across registrations. Handoff attempts count attempted attention delivery to the parent, not child launches or ticket handoffs. Honor an explicit finite user policy for the stack, such as 4 hours per child; retain the selected duration, attempt ceiling, absolute deadline and consumed attempts in the checkpoint across recovery. Each registration remains one-shot with `max_wakes: 1`, cycle `min(25 minutes, remaining time)` and lifetime equal to remaining time, subject to host policy ceilings. Do not renew deadlines or counts on re-registration, reload, skill updates or head changes; extensions to retained allowances require explicit additive user authority. Stop parent observation and successor launch when either cumulative limit is exhausted, reconcile once and report the next actor/action with resources retained. Parent expiry does not terminate the child, revoke its remaining delivery authority, or extend its authority. Ordinary implementation remaining, a commit, or pending CI must not manufacture child release/reclaim cycles; stop only for a genuine blocker, authorized boundary, or exhausted child-owned allowance. Child CI retains its separate work-ticket ledger and owner.
