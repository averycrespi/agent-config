...
- Use `shape-ticket` when the requested outcome is creating or revising a Plane delivery contract.
- Handle requested design or implementation-planning revisions directly within their authorized scope.
...
Do not edit the challenged artifact by default. If revision is requested, make the smallest changes needed to address the findings or recommend a separately scoped revision when the repair exceeds the authorized work. Preserve sound decisions and avoid turning a plan or design into a line-by-line implementation script.
