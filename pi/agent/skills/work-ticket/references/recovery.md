...
If preparation persisted but attachment did not, inspect Background receipts and registration evidence to determine whether start happened. Bind a proven matching registration with `ci attach`, even if already terminal, then reconcile. Display name alone is not identity proof. If the receipt is unavailable, prove the originating registration absent/inactive before `ci recover`. Missing UUIDs do not permit replay. If identity/liveness remains unresolved, retain intent and stop for user reconciliation.
