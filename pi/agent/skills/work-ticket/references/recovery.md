...
Read the persisted CI record, registration intent/Monitor ID, prior receipts, and allowances before registering anything. Monitor is session-branch-bound: reload/navigation/shutdown invalidates observation without automatic restart. Inspect the originating branch's receipt and confirm the watched PR/head; a missing receipt in a different branch does not prove the old observer absent. A same-session active matching watcher may remain in place; do not duplicate it. Cancel a mismatched live ticket watcher and reconcile its terminal receipt before changing identity or allowing a replacement. Never cancel unrelated monitors.

If `ci prepare` persisted but attachment did not, reread Monitor receipts and registration evidence to resolve whether start happened. Bind a proven matching registration using `ci attach`, even if its terminal receipt arrived before recovery, then `ci reconcile`. If its receipt is unavailable, prove the originating registration absent/inactive before `ci recover`. Do not replay an uncertain start merely because the checkpoint has no UUID. If identity/liveness cannot be established, retain the unresolved intent and name the user action needed; do not register again.
...
Older schema-v2 CI checkpoints without watcher fields remain readable. Charge any legacy open waiting interval before `ci prepare`; paused records retain consumed time. Stop a proven ticket-owned legacy polling Loop before adopting Monitor, but never commandeer an unrelated Loop. Neither new heads, replacement watchers, nor session restarts replenish allowances. An exhausted allowance requires an explicit additive exception and actionable handoff; “resume” alone is not an unlimited extension.
