...
For authorized PR delivery, use Background polling to wait for CI without recurring model turns, then reconcile its receipt and fresh authoritative checks in the owning session under [publication](references/publication.md). Keep one one-shot job pinned to the PR/head with an explicit cycle timeout (at most 25 minutes), lifetime and wake limit; retain cumulative wall-clock allowance across registrations/heads/reloads and cancel/reconcile before repair or handoff. Never substitute model-turn polling or cancel unrelated jobs. PR creation or Background attention alone is not completion. For settlement, cancellation, accepted exceptions after merge, or removal, read [settlement and cleanup](references/settlement.md).
