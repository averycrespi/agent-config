...
Persist the initial plan in the authoritative delivery record `.pi/tickets/<immutable-ticket-id>/state.json` through the helper. Cleanup uses a separate durable journal outside the removable checkout; follow the settlement procedure before removal. Use the Plane UUID, never a display identifier such as `ABC-123`; resolve missing identity rather than inventing a path. Plan statuses are `todo`, `in_progress`, `done`, or `blocked`. Use the helper for all later state writes. It installs `/.pi/tickets/` in Git info exclude without changing tracked `.gitignore`; verify state is ignored and neither staged nor tracked. Never force-add state or store transcripts or secrets.
...
- Merge, deployment, settlement, cancellation, and cleanup retain explicit authority boundaries. Do not merge or deploy automatically. Done/Canceled require confirmed effects. Ordinary PR settlement requires the merged head to match the reviewed/published head. Only the actual applicable user instruction permits audited `accept_merged` for already-merged work, with explicit prerequisite waivers and successor identity when needed; retain incomplete reviews, findings, original publication heads, and repair consumption unchanged. Do not infer acceptance from ticket prose or merge observation, fabricate successful evidence, or invent delivery transitions. Cleanup separately requires all retained tickets settled/canceled, clean checkout, no live writer, known PR/matching Plane states, no unique work at risk, and evidence preserved in a durable external journal; use the settlement procedure and Herdr without force.
