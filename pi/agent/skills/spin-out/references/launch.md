...
Use this procedure for standalone [spin-out](../SKILL.md) and explicitly authorized historical reconciliation. New bound coordination and [work-stack](../../work-stack/SKILL.md) use [Coordinate spawn](../../../extensions/coordinate/README.md), which reuses the finite mechanics below. The active workflow owns delegation authority and observation policy; this reference grants none. Resolve it from the conversation and retained agreement/index before creation: calling spin-out within active Coordinate or work-stack does not select standalone mode, even for one worker. Standalone mode needs no coordinator/index and leaves question UI unchanged. Managed mode requires the assignment/parent reference and [decision contract](decisions.md). Never infer delegation from task size or potential parallelism.

For new managed launches use Coordinate spawn after resolving authority and preflight. Its private implementation owns launch persistence; there are no agent-facing launch or bookkeeping scripts. Keep the manual procedure below as the standalone contract and explicit reconciliation reference, not a replay fallback.

## Preflight

Require scoped delegation authority covering the assignment, local branch/worktree, Herdr workspace, handoff and Pi process. A covering allowance needs no per-launch approval. Distinguish child execution/commit/publication authority from launch authority. Do not focus or remove resources without authority. Coordinate automatically records intent before each creation/start/submission and confirms its exact identity afterward; unresolved effects remain unresolved, never replayed. Do not edit its record or reproduce its private mechanics in shell. For standalone work, preserve consequential facts in the existing handoff/checkpoint. Exit zero or a shell `&&` chain is not execution confirmation; missing/malformed evidence requires reconciliation, never automatic replay.
...
3. Verify automatic listening on the coordinator's full session-ID mailbox **before the task prompt**. Include Mailbox loading in child bootstrap and preserve the actual worker session identity for bidirectional follow-up. No Monitor registration or Script permission is required.
...
The coordinator's delivery TODO remains open until the assigned completion boundary is evidenced and accepted under the active workflow. Launch completion is only an intermediate milestone, never delivery or acceptance. Let automatic Mailbox delivery resume the coordinator while work is pending; do not model-turn poll or mirror worker CI.

If the contract or messaging readiness is missing, retain unprompted resources and report blocked/uncertain launch with the missing evidence and next actor; no replay or restart. If submission may already have occurred, retain its uncertainty and reconcile the existing worker instead of calling it unprompted or sending again. Missing execution confirmation likewise retains the original submission and owner. Preserve authority and before-effect/readback gates during reconciliation; do not invent coverage or replenish allowances.
