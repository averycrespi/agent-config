...
For managed launches, use the [saved prepare/submit recipe](../../coordinate-repo/references/launch-script.md) after resolving authority and preflight below. The same definition carries one launch identity across both phases; the coordinator attaches existing mailbox observation between them. Keep the manual procedure below as the contract and explicit reconciliation reference, not a fallback that replays an uncertain Script. Standalone launches retain the manual procedure.
