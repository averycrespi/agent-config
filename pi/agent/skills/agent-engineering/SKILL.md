...
9. **Sticky completion + bounded fix loops.** Allow a small, explicit number of verifier-driven fix rounds when the workflow has a `fix` phase. After the cap, or once a task/phase reaches `done`, there is no automatic edge back to implementation: remaining findings become known issues. A workflow may support an explicitly user-authorized local follow-up that preserves history and repair consumption and invalidates stale evidence; this is not permission to reopen PR or settled lifecycle states. Without this, models perpetually nitpick on style. The `pi-supervisor` "5-strike lenient mode" is a useful reference point.
