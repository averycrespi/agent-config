...
In this repo, `pi/agent/extensions/subagents/` provides background-only one-child `subagent` delegation with explicit read, write, shell, MCP, and web capabilities; coordinated read-only fan-out belongs to background-only `workflow`, while Script retains foreground by default. `work-ticket` keeps implementation in its owning session, with compact recovery checkpoints under `<git-common-dir>/pi-ticket-checkpoints/`, artifact references, and explicit authority. It reviews before publication and uses session-bound Monitor polling for bounded read-only CI waiting without recurring model turns. The owning session reconciles host receipts and fresh checks before repair or promotion; cumulative monitoring wall-clock allowance includes background polls and survives heads/reloads. The helper protects ownership and retained allowances, not delivery-state gates; scoped user exceptions preserve failed/incomplete evidence, and new delivery labels alone do not require duplicate review.
