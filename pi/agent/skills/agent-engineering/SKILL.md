...
| Would multiple agents write to the same checkout concurrently?              | **Don't.** Actions carry implicit decisions; parallel writes fork micro-decisions that conflict at merge. Use one sequential writer, or worktree-per-task with no overlap. |
| Is implementation bounded by one task, explicit AC, and independent checks? | One sequential writable subagent can act as a context firewall while the orchestrator retains state, evidence, and commit authority.                                       |
