...
9. **Sticky completion + bounded fix loops.** Allow a small, explicit number of verifier-driven fix rounds when the workflow has a `fix` phase. After the cap, or once a task/phase reaches `done`, there is no automatic edge back to implementation: remaining findings become known issues. A workflow may support an explicitly user-authorized local follow-up that preserves history and consumed repairs while invalidating stale scope evidence. New scope may receive explicitly authorized additive repair allowance, never an automatic reset; this is not permission to reopen PR or settled lifecycle states. Without this, models perpetually nitpick on style. The `pi-supervisor` "5-strike lenient mode" is a useful reference point.
...
In this repo, `pi/agent/extensions/goal/` is the live example of durable objective steering and conservative completion evidence, while `pi/agent/extensions/subagents/` provides profile-routed delegation with explicit read, write, shell, broker, and web capabilities. `work-ticket` keeps implementation in its owning session, with a durable working plan and revision/scope-bound evidence in `.pi/tickets/<ticket-id>/state.json`, explicit authorization boundaries, and optional Loop continuation. Local helper receipts and opt-in content-equivalent evidence reuse reduce bookkeeping; scope-aware review retains qualification limitations without waiving applicable requirements. Goal remains a separate general-purpose objective primitive; it is not the ticket workflow's scheduler.
