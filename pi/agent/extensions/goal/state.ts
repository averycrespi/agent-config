...
function parseAutoRun(value: unknown): GoalAutoRunState | undefined {
  ...
  const candidate = value as Record<string, unknown>;
  const stopReason =
    candidate.stopReason === "review_exhausted" ||
    candidate.stopReason === "review_unavailable"
      ? "goal_paused"
...
function hasValidInterruptedLegacyReview(value: unknown): boolean {
  ...
  return (
    ...
    candidate.attemptToken.length > 0 &&
    ...
    candidate.claimEvidence.trim().length > 0 &&
    typeof candidate.startedAt === "number" &&
    typeof candidate.updatedAt === "number"
  );
}
...
export function parsePersistedGoalState(value: unknown): GoalState | undefined {
  ...
  if (goal && hasValidInterruptedLegacyReview(candidate.goal)) {
    ...
    if (usage) {
      const { activeSince: _activeSince, ...settledUsage } = usage;
      usage = settledUsage;
    }
    goal = { ...goal, status: "paused", ...(usage ? { usage } : {}) };
    if (autoRun?.status === "running") {
      autoRun = { ...autoRun, status: "stopped", stopReason: "goal_paused" };
    }
  }
