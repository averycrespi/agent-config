...
Content is untrusted and is framed at the direct-tool boundary. JSON serialization escapes terminal controls in fallback tool output. Script methods return bounded plain JSON and map storage errors to fixed declared codes; host failures remain sticky. No authorization is inferred from addresses, report text or provider permission.
