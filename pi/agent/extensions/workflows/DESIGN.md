...
Opt-in `retain: true` review execution captures the Git HEAD/binary-delta identity before launching, then sends the unchanged structured result, original report string, source and supplied scope to `_shared/delivery-artifacts.ts` before formatting/spillover. Final identity is compared rather than rebound; changed/unknown identity remains a warning. Retention failure never reruns the workflow or discards the original result. This is an immutable evidence artifact, not delivery state or a general successful-run journal.
