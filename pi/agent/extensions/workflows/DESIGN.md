...
A Background-owned signal closes runtime admission and drains admitted calls through existing termination. Reporting/storage failure aborts this same signal path rather than escaping an IPC event callback or stranding work. Runtime logical-call settlement counts (not per-attempt activity) drive monotonic shared activity snapshots. An optional canceled subset preserves historical unsuccessful counts while allowing disjoint display categories; the host ledger supplies reported cumulative tokens across retry attempts. Uninvoked parallel thunks are not predicted children. Phase changes are actual sandbox messages. One final notification is owned solely by Background.
...
Background calls show action and saved or statically extracted literal inline name; inspect/cancel/dismiss show a short requested ID. Extraction never evaluates source. Controls render the shared display-only `background/render.ts` flush-left outcome and expanded retained evidence. Bounded failure causes are retained with outcome references for inspection without fetching files; metadata may contain sensitive error text and follows the sidecar's retention contract. Immediate list/validate also render a call row; the collapsed result holds only the saved count or validation outcome. Background run and controls use retained execution summaries; expansion adds diagnostics without mistaking admission for completion. Workflow agents reuse the shared two-line subagent grammar: stable identity and statistics first, then compact `profile (capabilities)` policy, workflow timeout metadata, and volatile activity last. Fixed workflow capabilities map to `fs`, `mcp`, and `web`; empty sets are omitted. Agent rows preserve chronological start order regardless of status, with the newest at the bottom; settled-history limits hide rows without reordering the visible ones. List inventory rows move to expansion, while validate results remain one line. Expansion adds typed failures, workflow logs, retained and recovery paths, the list store path and invalid-entry diagnostics, or the validated source path. Tool identity is emphasized in the call; renderer-authored tool lines use commas or parentheses rather than middle-dot separators. Status prefixes use explicit words and semantic colors, never icons. Inventory validity is `valid`/`invalid`, distinct from execution `succeeded`/`failed` and `completed with failures`. Dynamic data is control-normalized, bounded, and width-aware. Prompts, raw scripts, secrets, and compressed content are not rendered.
