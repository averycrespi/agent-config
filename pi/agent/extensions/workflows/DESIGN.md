...
- `config.ts` loads timeout, concurrency, budget, expanded-agent display, fixed-tier, and single-store settings through shared precedence helpers.
...
Workflow progress is in-memory for one foreground call. Snapshots retain metadata, phases, logs, agent activity, timings, previews, host-resolved per-agent timeouts, whether a timeout came from a valid per-call override, normalized terminal metadata, and separate final-agent/logged-branch/settled-branch failure counts.
...
`workflow-tool.ts` merges runtime snapshots with final spawner-owned states by request ID and carries the normalized settled-row display limit in run details. `display.ts` follows the repository tool-row grammar: every action has a stable source-free call summary; collapsed failures show one compact authoritative cause/count line; expanded runs separate the call and workflow header, show every running agent plus the configured number of most recent settled agents, and place a dim hidden-history summary before visible rows. Settled rows preserve their original order after selection. Routine rows suppress the repeated default timeout, while valid per-call overrides remain inline and expanded diagnostics still reveal typed failure metadata, recent logs, inventory, validation source, and finalized paths. Dynamic display fields are bounded and control-character-normalized before the shared width-aware renderer truncates each logical line. Compressed contents are never read by rendering. Script logs are capped at 100 entries × 2,000 characters and phases at 100 entries × 200 characters in both the sandbox and host, preventing unbounded IPC/state/TUI amplification. Final output uses shared safe stringification and spillover.
