...
The call renderer is intentionally empty so collapsed tool output has exactly one aggregate result line. Each header includes the explicit action (`run`, `list`, or `validate`) after the tool name. Expansion must preserve that line byte-for-byte before adding details. Workflow agents reuse the shared two-line subagent grammar: stable identity and statistics first, then compact `tier:thinking (capabilities)` policy, workflow timeout metadata, and volatile activity last. Fixed capabilities map to `fs`, `shell`, `broker`, and `web`; empty sets are omitted. Typed failures and diagnostic paths remain additional detail. Expanded agent rows preserve chronological start order regardless of status, with the newest at the bottom; settled-history limits hide rows without reordering the visible ones. Expanded output also shows logs, inventory, source paths, and diagnostics. Tool identity is emphasized; separators and supporting metadata are muted. Dynamic data is control-normalized, bounded, and width-aware. Prompts, raw scripts, secrets, and compressed content are not rendered.
