...
async function parallel(thunks, options = {}) {
  ...
  async function runOne() {
    while (next < thunks.length) {
      ...
      if (typeof thunk !== "function") throw new Error("parallel entries must be functions");
      try { results[index] = await thunk(); }
      catch (error) { post({ type: "log", level: "error", message: error?.message || String(error) }); results[index] = null; }
    }
  }
