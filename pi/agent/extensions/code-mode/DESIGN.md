...
The gateway client optionally performs this stronger admission without changing ordinary three-argument `callTool` behavior. Raw redacted `CallResult` objects preserve unknown metadata fields through IPC. Provider `isError` marks the host trace failed but still resolves the raw value to the guest; gateway exceptions reject with metadata only. The host never derives code/reason/uncertainty from remote prose or guest exceptions. Gateway errors carry a shared host-only symbol brand: use the API's `isGatewayError` predicate rather than constructor identity, since Pi loads sibling extensions with separate module caches. Plain JSON error-shaped objects cannot carry that brand.
...
Pi session history and final spills are the only extension-side retained content. Do not add raw tracing, guest logging, error previews, hidden result handles, or source retention without revisiting the context/retention contract. The renderer shows the caller-supplied description and host metadata only; the returned JSON remains in tool-result content for the model. The required description is display-only outer-call data, never passed to the guest or treated as authorization. Its published schema uses a portable `\S` pattern: Codex rejects Unicode property escapes in tool schemas even though Pi's local JavaScript validator accepts them. Execution rejects whitespace/Unicode-format-only descriptions before accessing the gateway or starting a child. Sanitize and bound it before styling; retain the generic label for historical or partial arguments without a usable description.
