...
Calls show the quoted search query or fetch URL, including its path and useful query parameters; a separate flush-left result shows search counts or warnings, while routine successful fetches have no collapsed result. Fetch labels remove userinfo and fragments, redact recognizable secret query parameters (tokens, passwords, API keys, credentials and signatures), and truncate with an ellipsis to available width, with a 4,096-character safety cap. This is not general secret detection: arbitrary path or query values may still be sensitive. Counts and spill indicators replace raw page/snippet/title previews. Successful spill summaries use muted `output truncated · full response saved to file`; a muted middle dot separates them from any other summary. Expansion labels the file path `Full response:`. Single outcomes and diagnostic prose are unchanged. Original URL arguments remain unchanged in model/session context. Failed uncertain calls retain both failure and no-replay wording before any target. Expand for bounded original trust-framed evidence; full model-facing content is unchanged. All display strings are sanitized before theme styling.
...
Successful `web_search` and `web_fetch` results are wrapped in a short `BEGIN/END UNTRUSTED EXTERNAL ... CONTENT` envelope. Remote provider/fetch error messages are framed too, while the collapsed renderer shows search counts or an explicit failure/uncertainty/spill warning, omits routine fetch-success results, and retains framed diagnostics on expansion. The envelope reminds the agent that fetched web pages, search snippets, GitHub contents, PDF text, and remote error bodies are external data rather than instructions. Delimiter-like lines inside external content are escaped. Content is wrapped before large-output spillover so the persisted file retains the same trust boundary.
