...
Tool rows use bold tool names, plain-text targets and muted metadata; call argument names never expose values. Collapsed results are one contextual line: search shows muted shown/matching counts separated by middle dots; routine describe/call success is silent, not execution acceptance. Retained-output warnings form a separate middle-dot-delimited field when another summary is present. Raw response/description previews stay expanded. Explicit `unknown effects; no replay` outranks optional targets; failed uncertain requests also retain `failed` rather than hiding the operation failure. Other failures say `request failed`; expand for recognized rejection reasons, labelled untrusted guidance, invocation IDs and diagnostic paths. Running/error/settled states use semantic theme tokens and meaningful words. Expansion retains framed content with a 2000-line display bound and spill references; model-facing payloads are unchanged. All dynamic text is sanitized before styling and rows truncate rather than wrap.
