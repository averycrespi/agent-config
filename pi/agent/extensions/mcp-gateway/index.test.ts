...
test("real Pi loader is inert by default; CLI flag enables tools at session startup and shutdown cancels them", async (t) => {
  ...
  loaded.runtime.flagValues.set("mcp-gateway", true);
...
test("gateway does not replace another extension's meta-tools when explicitly enabled together", async (t) => {
  const f = await fixture(t);
  const loaded = await discoverAndLoadExtensions([path], f.dir, f.dir);
  const extension = loaded.extensions[0];
  loaded.runtime.flagValues.set("mcp-gateway", true);
