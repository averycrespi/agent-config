...
`GatewayError` exposes locally generated summary/code, optional validated rejection reason and invocation ID, uncertainty, and optional bounded external guidance. Consumers crossing a model-context boundary must not copy guidance or arbitrary exception messages automatically: those can contain intermediate data. The Script provider forwards only declared safe failure codes and outcome uncertainty. Provider `isError` results remain raw data; consumers must retain a host-observed failure separately from guest control flow.
...
| ---------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `mcp.call(name, args)` | Invoke one discovered external tool using a nonempty name and JSON argument object | Complete redacted MCP envelope, including `content`, `structuredContent`, `isError`, and unknown metadata |

The method neither parses text nor changes application semantics. The direct Pi adapter separately frames/presents the envelope; its presentation is not the guest value. Runtime discovery is authoritative for positional argument schemas; provider semantics are documented here, while shared bounds and execution rules live in [Script](../script/README.md).

### Example

Discover the method schema first:

```js
script({
  action: "describe",
  description: "Inspect MCP composition methods",
  providers: ["mcp"],
});
```

Discover exact external names with `mcp_search` and inspect their schemas with `mcp_describe` before composition. Previously inspected names/schemas may be reused unless evidence indicates a change; fresh host admission still runs.
...
The example name is illustrative, not a discovered tool; replace it with an inspected name and matching arguments before execution.

### Permissions and effects

The adapter uses `createGatewayAccess` on the same owned client, preserving fresh catalog/schema/read-only admission, grants, endpoint/credential ownership, response bounds and per-call deadlines. Gateway permission is not user approval: obtain applicable authority before mutations. External tools may have durable effects. There are no automatic grants, approval polling, invocation retries, replay or synthetic nested Pi tool hooks. No new provider logs or spills are written.

### Failure and lifecycle

Provider `isError` resolves unchanged but forces host failure. Known gateway exceptions reject with a declared safe `code` and `outcomeUnknown`; Script retains the code independently of guest catches. Raw exceptions and rejection guidance are suppressed. Rejection reasons/invocation IDs are not included in this minimal provider error contract; direct tools retain their richer diagnostics. Unrecognized exceptions conservatively become unknown provider failures. Script accounts handler entry as potential dispatch, even for provider-internal preflight rejection, so effects/partial flags can conservatively overstate effects. No gateway envelope is invented for a transport/admission failure.

The Script deadline/signal further narrows execution. Configuration changes and shutdown dispose the registration, cancelling **all** executions selecting it, including runs between calls. New executions select a fresh registration; old executions cannot adopt new authority.

Cancellation is not rollback. Inspect Script's host status/partial/unknown accounting, not just returned JSON. Script retains its own smaller final-output and IPC bounds; an over-limit result fails rather than replaying or truncating an envelope into apparent success.
