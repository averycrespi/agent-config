...
export function buildSandboxSource(
  ...
  const setup = `(() => {
    ...
    for (const namespace of Object.keys(bindings)) {
      const methods = Object.create(null);
      for (const method of bindings[namespace]) {
        Object.defineProperty(methods, method, { value: (...args) => call(namespace + '.' + method, args), enumerable: true });
      }
      Object.defineProperty(globalThis, namespace, { value: Object.freeze(methods) });
    }
      ...
      try {
        function snapshot(v, depth) {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            if (array && (!isFinite(+key) || +key < 0 || +key >= descriptor(v, "length").value || +key % 1 !== 0 || '' + (+key) !== key)) throw 0;
