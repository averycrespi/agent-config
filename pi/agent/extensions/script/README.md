...
Control actions omit `providers`, `source`, `name`, `args` and `execution`. `executions` omits results; `inspect` returns one bounded result plus original executor accounting. Cancellation requests abort, not rollback. Dismissal rejects active work and clears terminal attention without deleting evidence. Background automatically sends a bounded terminal notification with an inspection reference, including failures and interruptions; the model need not poll. Its one-line below-editor row includes elapsed time since admission, frozen at settlement, and respects drafts, active turns and sibling widgets. Successful inspection includes retained elapsed time; unavailable child/token telemetry is omitted. See [notification integrity, retention and limits](../background/README.md) before interpreting handoff or consumption as completion.
...
Background admission uses the saved name or description in its call row; inspect/cancel/dismiss use a short execution ID. Background controls use an action/identity call row and a separate flush-left status or uncertainty line, never full IDs or a misleading provider-selection placeholder. Expand for retained records and framed evidence. Admission is not completion; cancellation is not rollback; dismissal retains evidence. The foreground run/progress and provider-discovery renderers below are unchanged.

The call row shows the action, nonsecret description, and an optional parenthesized provider selection or discovery scope. Missing selection is omitted rather than labelled pending. Execution uses `providers: web, mcp` or `providers: none`; discovery uses `scope: web, mcp` or `scope: all` for `[]`. Here, **all** means currently registered, host-permitted providers, not unrestricted access. Selection is a request, not proof of permission, availability, or user approval. More than three selected names use a `+N more` suffix; expand for the full selection.
