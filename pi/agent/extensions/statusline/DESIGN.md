...
`renderFooterLines` prepends exactly one full-width `borderMuted` rule for positive widths, independent of whether Background or any other widget is mounted. The footer owns the boundary with the above content; no separate widget, visibility coordination, or timer is involved. Non-positive widths retain the empty-line fallback. The content-only `renderFooterLine` helper remains unchanged.
