...
| `cronEnvironment`            | `{}`                             | `SCHEDULED_TASKS_CRON_ENVIRONMENT`              | Environment variables scoped to the managed cron scheduler CLI and descendants; env override is a JSON object of string values.          |
...
- `/scheduled-tasks-run-claimed <task-id> <run-id>` is a manual/debug and compatibility wrapper for an existing claimed run; normal unattended launches use the deterministic repo-local run-claimed CLI instead.
- `/scheduled-tasks-logs <task-id>` shows latest run status, artifact paths, and bounded output/log tails.
- `/scheduled-tasks-doctor [task-id]` validates config, root, managed crontab installation status, latest tick status, command health, task files, last runtime state including run duration when lifecycle metadata is available, next scheduled run time for enabled tasks, and current task-lock diagnostics.
- `/scheduled-tasks-tick [--dry-run]` is a manual/debug wrapper that runs one scheduler tick and appends a compact entry to `state/ticks.jsonl`. `--dry-run` reports decisions without mutating task state, acquiring task execution locks, spawning child Pi, or writing run artifacts. Managed cron uses the deterministic repo-local tick CLI instead of this slash command.
...
<root>/runs/<task-id>/<run-id>/launch.stdout.log
<root>/runs/<task-id>/<run-id>/launch.stderr.log
...
Managed cron invokes the deterministic repo-local tick CLI through the repo-local `node_modules/.bin/tsx`; `/scheduled-tasks-tick` remains an interactive wrapper over the same scheduler logic. Each tick scans enabled tasks, initializes missing `nextRunAt` to the next future occurrence without immediate execution, and skips missed schedules outside the 90-second due window unless the task opts into catchup. Cron expressions use the scheduler process's local/current timezone and standard five-field day-of-month/day-of-week behavior: if one field is unrestricted, the restricted field controls matching; if both are restricted, either field may match.
...
`/scheduled-tasks-install-cron` manages one marked block and leaves unrelated crontab entries untouched. The managed cron line changes to the project cwd captured when the command is run, then invokes the repo-local TypeScript scheduler CLI:
...
All configurable values in the cron command are shell-quoted. `piCommand` is still treated as the executable path or command name for final child Pi task execution, not for scheduler orchestration. `cronEnvironment` is merged key-by-key across default, global, project, and environment config layers, then emitted inline after `cd ... && env`, so values apply only to the managed scheduler process and its children, not to unrelated crontab entries. Do not put secrets in `cronEnvironment`; crontab entries are not secret storage.

Cron installation fails before writing crontab if the repo-local `node_modules/.bin/tsx`, `tick-cli.ts`, or `run-claimed-cli.ts` files are inaccessible. Install this repo's dependencies first, then rerun `/scheduled-tasks-install-cron`. After code updates that change the managed block, reinstall explicitly and prove it manually; the extension never rewrites live crontab except when the install/uninstall commands are invoked.

The unattended scheduler control plane must not depend on Pi slash-command dispatch. Cron uses `tick-cli.ts`, claimed-runner launches use `run-claimed-cli.ts`, and only final task execution launches Pi through the configured `piCommand`. Keep the slash commands as manual/debug wrappers over the same functions.

Manual reinstall/proof sequence after a scheduler cron update:
...
Only run the install command when you intentionally want to rewrite the live managed crontab block.
