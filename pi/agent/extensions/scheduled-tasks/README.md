...
- `/scheduled-tasks-doctor [task-id]` validates config, root, managed crontab installation status, latest tick status, command health, task files, last runtime state, next scheduled run time for enabled tasks, and current task-lock diagnostics.
