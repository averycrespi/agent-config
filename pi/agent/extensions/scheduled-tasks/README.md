...
Catchup is coalesced, not replayed. A task with `catchup: true` runs at most one make-up run when the scheduler notices missed occurrences, even if it missed multiple scheduled times. One tick claims at most `maxCatchupRunsPerTick` catchup runs globally; extra eligible tasks are reported as `catchup_deferred` and keep their existing `nextRunAt` for a later tick. Active scheduled runners are capped globally by `maxConcurrentScheduledRuns`; already-active `claimed`, `launched`, or `running` runs count across ticks. When the cap is reached, due tasks are reported as `concurrency_deferred` and keep `nextRunAt` unchanged. Per-task concurrency is still limited by each task lock.

`/scheduled-tasks-tick` returns a scheduler-level summary with `status`, `timestamp`, `dryRun`, `claimed`, and `skipped`. Claimed entries report launch/claim outcomes such as `launched` or `launch_failed`; final task success or failure is visible later through `run.json`, `result.json`, logs, and doctor/log commands. Scheduler lock contention is reported as `status: "locked"` with no fake task ID. Dry-run ticks report what would initialize, miss, catch up, defer, or run without writing task state, acquiring task execution locks, spawning child Pi, or writing run artifacts; they still append a compact tick-log entry. If a due task is already running, the scheduler reports a task-level locked skip and leaves `nextRunAt` unchanged so later ticks can retry while the due time remains inside the grace window. Scheduler locks stale after 5 minutes. Task locks stale after the task timeout plus a 5-minute cushion; same-host dead-PID locks can be recovered after a 30-second safety floor. Recovery marks associated active lifecycle metadata as `stale_recovered` before new work is launched.
