...
The launch gate inspects the actual process-local Monitor receipt/source match and Mailbox availability, not a caller-supplied green claim. After preparing the worker it establishes its exact session binding and reporting identity before one task prompt. Success requires task-correlated input plus fresh execution evidence. Process readiness and `agent_prompted` are insufficient.
