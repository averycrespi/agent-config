...
Coordinate owns durable delegation facts, not agent orchestration. Mailbox transports reports, Monitor owns bounded attention, TODO remembers unresolved obligations, and child checkpoints own execution/evidence.
...
- `launch.ts`: capability/base preflight, assignment persistence, actual Monitor inspection and child binding between prepare and submit.
...
New state contains only role binding, assignment identity, one launch brief including committed base and task constraints, worker location, launch status/unresolved operation and acceptance references. Monitor references contain IDs only. Completed intermediate intents are cleared; raw Herdr envelopes, redundant prompt copies, ACK/report histories, wellness, questions/answers/relay ledgers and secondary supervision accounting are not generated. Historic sections remain readable and are not deleted or adopted automatically.
...
Matching explicit Monitor starts and spawn inputs retain observer IDs. Read actual receipts for launch gates, display and disable checks. No scheduler, automatic registration/rearm, copied budgets or inferred renewal. Historical unresolved accounting is a reconciliation blocker, not revived authority.

## Rendering

A persistent supported below-editor widget is mounted once while visible, updated in place on lifecycle/tool/mailbox/Monitor events, and removed when disabled or shutting down. It owns no timer, border or footer behavior. Stale async refreshes cannot repaint a newer state. Labels are sanitized before styling and fitted to one row. Counts derive from retained assignments and inbox inspection; pending messages are not inferred questions or worker activity. Supervision warnings require outstanding assignments.
...
Regression tests cover bindings, original mailbox reuse, request context replacement, read-only status, ownership/acceptance, actual Monitor coverage, launch partial failures and no replay. Pure display tests cover hostile labels and bounded widths. Fake Herdr tests cannot qualify live Pi/Herdr delivery, focus, draft handling or model obedience; keep these separate from deterministic evidence and require explicit live-test authority.
