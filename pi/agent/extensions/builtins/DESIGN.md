...
- Bash failures restore the pre-CONFIG-33 first-nonempty-line preview, in error color, with no added classification. An empty failure falls back to `bash failed: <command>`. Expansion deliberately shows the same compact error or success tail. Other builtin collapsed errors retain their terse classifications.
- The four non-Bash builtins honor expansion with bounded sanitized original text and available full-output paths. Shared expansion limits disclose clipped text; recognizable credentials are redacted before styling in both views. Tool truncation/limit warnings stay visible even for otherwise silent reads. Expansion never fetches files or changes model-facing results or stock execution.
