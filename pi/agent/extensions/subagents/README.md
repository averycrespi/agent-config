...
Controls accept no agent or execution options. List omits result bodies; inspect retains input-aligned outcomes, structured values, prose, diagnostics and reported usage. Large results expose a JSON `resultFile`; use `read` to inspect the complete retained result. A single child has one Background row showing state, intent, reported token usage and elapsed time instead of redundant 0/1 counts. Profile and live phase/tool detail remain inspectable. Historical batches without disjoint cancellation metadata retain settled/unsuccessful counts. One terminal notification follows. Continue independent authorized work while awaiting notification, without duplicating the child's investigation or changing files under its review. Yield when no useful independent work remains; do not poll for completion. Per-child output belongs in inspection, not extra widgets or notifications.
...
Subagent run/list/inspect/cancel/dismiss use contextual one-line summaries. Lifecycle words keep their semantic colors even beside an uncertainty warning: succeeded is green, failed red, canceled yellow. Ordinary success omits generic effects-may-persist display boilerplate; unknown effects remain visible without replacing the lifecycle state. Inspect/cancel/dismiss calls show the full requested ID; wake expansion and inspection retain it. Per-child output and retained evidence stay expanded; admission, cancellation requests and attention dismissal are distinct. The shared widget/notification uses singular `subagent` when typed progress identifies one child, otherwise `subagents`. New admissions use the child's bounded intent as their widget and wake label without a short ID by default; historical batch labels retain their original child count. Rendering never consumes or accepts results.

The direct run returns an admission receipt; use inspection for the retained result, including child status, usage, diagnostic paths and structured/prose output. Child progress rows, including historical layouts reused by Workflows, use explicit `queued`, `running`, `thinking`, `succeeded`, `failed` and `canceled` words with semantic colors, not status icons. Dynamic labels are control-normalized, bounded and width-aware; prompts and raw logs are not rendered in compact rows.
