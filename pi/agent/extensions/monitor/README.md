...
The former `sessions` provider and Unix session-event transport are removed. Use [automatic session Mailbox delivery](../mailbox/README.md#delivery-and-lifecycle) for cross-process coordination without Monitor setup. Optional Mailbox events carry addresses only. Messages survive closed sessions; session settlement is neither a report nor task completion.

[Coordinate](../coordinate/README.md#supervision-and-reports) owns assignments and exact acceptance; Mailbox transports reports and TODO retains unresolved actions. Monitor owns observation receipts and finite bounds, without a copied coordination accounting ledger. Child execution/CI budgets remain separate. Monitor's local settlement, message-admission, shutdown and navigation hooks remain intact. Unrelated external-state polling, including child-owned CI, is unchanged.
...
Before requesting user input, cancel the continuation job for affected input-blocked work and reconcile its receipt; do not leave recurrence retrying blocked work while waiting for a decision. Explicitly authorized read-only shared observation of independent workers may continue under the [nonblocking correlated-message contract](../../skills/spin-out/references/decisions.md#incorporation-and-questions). It does not open a modal wait, poll approval or resume blocked writes; ordinary interactive defaults and approval semantics are unchanged. There is no yield/resume/extend. After real user input, start a fresh immutable registration only within existing authority and caller-retained remaining lifetime/wake allowance. Cancellation cannot retract a Pi-owned follow-up. Count attempted/unknown handoffs conservatively, never reset consumed workflow budgets. The first delay begins on admission; only subsequent delays follow positively correlated settlement. Evaluators return wait/wake, never terminate a recurring job; cancel when no useful authorized work remains.

### Compound polling and events with explicit state

For explicitly authorized observations, events and polling share one fresh evaluator with optional bounded JSON state committed only on success. Never use evaluator state as the sole copy of unanswered questions or ACK messages from an evaluator. [Mailbox](../mailbox/README.md) owns ordinary session listening, batching and redelivery automatically; no Monitor job or Script evaluator is needed for inter-agent messaging.
...
Interactive wakes use the shared [one-line notification renderer](../_shared/README.md#asynchronous-custom-messages): `monitor timer elapsed timer demo`, for example. Type and observation reason precede actionable effects/interruption/coverage warnings and bounded identity. Reasons are `condition met`, `timer elapsed`, `timed out`, `evaluation failed`, `coverage lost`, and `budget exhausted`, without an `attention` prefix. Dispatch metadata does not establish mutation, so neither compact nor expanded notifications add generic `effects may persist` prose. Actual outcome uncertainty, interruption and coverage failures remain visible; original receipt metadata is retained. Timer elapsed means the requested continuation delay completed; timed out means the observation deadline expired. Neither means the continuation or watched task completed. Condition met and timer elapsed use success styling to indicate the configured trigger occurred normally, not watched-task completion. Timeout uses warning styling; evaluation and coverage failures use error styling. Actionable warnings remain independently warning-colored. A subtle `customMessageBg` distinguishes notifications without icons or padding. Essential status/warnings are qualified at 48 content columns and above; smaller widths remain bounded with full text available on expansion. Timer wording requires typed producer metadata; condition and timeout never imply watched-task success or failure. Evaluation failure, coverage loss and budget exhaustion remain distinct.
...
One stable, width-bounded row per visible job appears below the editor. Source and observation state lead immediately into bounded identity, followed by optional mechanism metadata, timing, counters and actionable warnings:

```text
monitor watching CI check · polling · next check 3s · timeout 12s
monitor watching Worker · polling + events · next check 21s · timeout 15m
monitor watching Reports · events · timeout 15m
...
`watching`, `checking`, and `scheduled` use accent; condition met and timer elapsed use success; timeout attention and awaiting settlement use warning; evaluation/coverage failures use error. Source is muted, names and numeric values use text, metadata labels are muted and separators dim; rows have no bold or background. Green means the configured trigger occurred normally, not watched-task success. Source, payload and evidence stay out of widgets. Muted `polling`, `events`, or `polling + events` follows the name while watching/checking; omit it for timers, pending attention, settlement waiting, or missing trigger metadata. Drop mechanism details before timing or actionable warnings under width pressure. Generic mutation warnings are not inferred from provider dispatch flags; this does not change receipt accounting or inspection metadata. Narrow rows reserve critical uncertainty before fitting optional names and telemetry, but keep any visible name adjacent to the state. Warnings follow the remaining fields; queued-status/clock/identity detail is dropped under pressure. At 64 columns, combined warnings can compact to `unknown/interrupted/gap/handoff?`; expansion/inspection retains full wording. Smaller widths remain bounded, with optional fields omitted first. Pending attention remains visible until handoff/suppression even after observation ends. Finished/canceled rows disappear after handoff/suppression. TUI mounts once and repaints in place at most once per second for countdowns; RPC uses string arrays and headless mode makes no UI calls. Older receipts without trigger metadata remain inspectable without guessing a polling/continuation mode.
