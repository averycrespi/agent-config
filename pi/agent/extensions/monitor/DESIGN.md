...
- `tool.ts` defines the schema, bounded receipt summaries, untrusted notification framing and compact width-aware rendering. Widget/tool rows do not read source, arguments or evidence. The widget includes every waiting/observing receipt and no terminal receipts, independently of capacity occupancy during cleanup/handoff. UI-local observation timestamps keyed by monitor ID and poll number delay the `observing` label by the shared two-second elapsed threshold; waiting/short polls render as `active`. Refreshes preserve the current poll's timestamp; settlement, removal and lifecycle clearing discard it. Countdown rounding is local to Monitor, not a change to the shared elapsed-time formatter or persisted receipts. Append one width-bounded `borderMuted` separator after the active group, never for an empty group. With no active receipts, remove the widget and stop its refresh timer; list/get retention and notification handling remain unchanged.
