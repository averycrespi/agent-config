...
Every admitted execution has one sanitized, width-bounded row below the editor: `<source> <state> · <effects warning> · <label> · <telemetry>`, with `script`, `workflow`, or singular/batch `subagent`/`subagents` as the source. Rows contain genuine lifecycle state, not simulated progress. Adapters may report aggregate settled/total and failed counts, or genuine workflow phase and settled/started logical-call counts. Workflow rows add elapsed time since admission; single-subagent rows replace the unhelpful 0/1 count with the centrally selected profile, admission-to-now elapsed time and live phase or active tool name (never its arguments). Per-child details remain in adapter inspection. Running rows repaint once a second without renewing deadlines or simulating progress. Terminal rows remain until observed notification consumption or explicit dismissal. The widget repaints in place without reordering sibling widgets. IDs remain in inspection rather than taking widget width from state, warnings or names. At 64 columns combined warnings can compact to `unknown/persist failed/handoff?`; inspection retains full wording.
