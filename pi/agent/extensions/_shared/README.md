...
- `delivery-artifacts.ts` — opt-in immutable review/Background evidence under `<git-common-dir>/pi-delivery-artifacts/{review,background}/`. Content-addressed JSON retains exact strings and host receipts (4 MiB per artifact), using real owner-controlled directories and mode-0600 staged writes. No automatic deletion, replay, remote publication or model transcription. References include SHA-256 and byte count. Review identity includes HEAD and a binary tracked-delta digest; stage intended new files first. This is local evidence, not proof of authority, remote truth or a stable live checkout.
