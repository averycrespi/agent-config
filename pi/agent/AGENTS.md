...
- **Ticket implementation is an explicit exception.** Requests to implement or resume implementation of a selected ticket under `work-ticket` authorize in-scope local commits unless the user excludes them. This does not authorize pushing, PR publication, history rewriting, or committing unrelated work.
