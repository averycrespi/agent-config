...
Use local repo tools first when analyzing local files or exported artifacts. Use MCP broker tools for read-only observability data, issue metadata, PR context, or remote metadata only when materially useful.

Do not make changes. Do not over-index on a single event. Prefer aggregate patterns over anecdotes, but include concrete examples for each important pattern.
