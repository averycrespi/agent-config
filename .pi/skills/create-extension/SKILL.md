...
- Render a stable call line: bold `toolTitle` tool name, muted action/metadata, plain `text` target. Use `toolSummary` and `displayLabel` from the shared render helper where applicable. Never echo raw scripts, prompts, secrets, message bodies or arbitrary output previews by default. Style partial state as warning, successful operations as success, and failures as error; retain explicit outcome words without color.
- Keep ordinary collapsed results to one unwrapped line, e.g. `mailbox send · persisted · result · not consumed · reports`. Prioritize unknown effects and no-replay guidance before optional targets; full IDs, inventories, logs, output and diagnostics belong in expanded results. Preserve contextual failures and honor both `context.isError` and semantic errors. A read is not execution success, persistence is not consumption/acceptance, ack is not task resolution, scheduled launch is not completion, and an answer is not approval. A pending human-choice form may remain multiline. Do not apply this compact grammar to the existing foreground Script/Subagents/Workflow run/progress renderers.
