...
- **My own skills** (`clarify` → `architect` / `specify` → `plan` → `challenge` → `/goal` → `review` → `complete-work`) — a stateless clarification and routing step; optional high-level architecture decomposed into bounded specifications; an acceptance-criteria-to-verification plan; a contrarian pre-implementation challenge pass; bounded autonomous execution via the native `/goal` command; then a non-destructive multi-dimension review before PR.
